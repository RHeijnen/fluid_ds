...
describe("<fluid-stat>", () => {
  it("renders label and value", async () => {
    const el = await fixture<FluidStat>(
      html`<fluid-stat label="Revenue" value="$10k"></fluid-stat>`
    );
  ...
  it("shows the change with a trend arrow and exposes a group label", async () => {
    const el = await fixture<FluidStat>(
      html`<fluid-stat label="Revenue" value="$10k" change="+12%" trend="up"></fluid-stat>`
    );
    ...
    expect(el.shadowRoot!.querySelector('[role="group"]')?.getAttribute("aria-label")).to.contain(
      "+12%"
    );
  });
  ...
  it("renders a downward trend in the accessible group label", async () => {
    ...
    expect(el.shadowRoot!.querySelector('[part="change"]')!.getAttribute("data-trend")).to.equal(
      "down"
    );
  });
  ...
  it("passes the a11y audit", async () => {
    const host = await fixture<HTMLElement>(html`
      <div
        style="--fluid-text-primary:#18181b; --fluid-text-secondary:#3f3f46; --fluid-success-text:#15803d;"
      >
