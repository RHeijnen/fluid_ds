...
describe("<fluid-empty-state>", () => {
  it("renders the heading", async () => {
    const el = await fixture<FluidEmptyState>(
      html`<fluid-empty-state heading="Nothing here">Body</fluid-empty-state>`
    );
    expect(el.shadowRoot!.querySelector('[part="heading"]')?.textContent).to.contain(
      "Nothing here"
    );
  });
  ...
  it("hides the actions wrapper when no actions are slotted", async () => {
    const el = await fixture<FluidEmptyState>(
      html`<fluid-empty-state heading="Empty">Body</fluid-empty-state>`
    );
  ...
  it("passes the a11y audit", async () => {
    const el = await fixture<FluidEmptyState>(
      html`<fluid-empty-state heading="No data">Add something to begin.</fluid-empty-state>`
    );
