...
describe("captureFluidFormState", () => {
  for (const type of ["checkbox", "number", "email", "date", "range", "color", "file"]) {
    it(`restores a focused native ${type} without invoking unsupported text selection`, async () => {
      const root = await fixture<HTMLDivElement>(html`<div><input type=${type} aria-label="Control"></div>`);
  ...
  it("includes a native control supplied as the capture root", async () => {
    const input = await fixture<HTMLInputElement>(html`<input value="before" aria-label="Root control">`);
  ...
  for (const explicitDocument of [false, true]) {
    it(`captures controls from the ${explicitDocument ? "explicit" : "default"} document root`, async () => {
      const input = await fixture<HTMLInputElement>(html`<input value="document state" aria-label="Document control">`);
  ...
  for (const shadowRootAsRoot of [false, true]) {
    it(`recurses through nested open shadows from a ${shadowRootAsRoot ? "shadow" : "host"} root and restores deep focus`, async () => {
      ...
      expect([input.selectionStart, input.selectionEnd, input.selectionDirection]).to.deep.equal([1, 4, "forward"]);
    });
  }
  ...
  it("includes a supplied host's shadow controls and reconciles submitted input state without events", async () => {
    const form = await fixture<HTMLFormElement>(html`<form><fluid-input name="note" value="server" required label="Note"></fluid-input></form>`);
  ...
  it("reconciles checkbox checked and indeterminate state without replacing its configured submission value", async () => {
    const form = await fixture<HTMLFormElement>(html`<form><fluid-checkbox name="choice" value="yes" required>Choice</fluid-checkbox></form>`);
  ...
  for (const fixtureTemplate of [
    ...
    html`<form><fluid-date-range-picker name="answer" start="2026-08-27" end="2026-08-28" typeable></fluid-date-range-picker></form>`,
    ...
    it("preserves a parsed control's uncommitted display draft without changing canonical form state", async () => {
      ...
      const host = form.firstElementChild as HTMLElement & { updateComplete: Promise<boolean>; value: string | null };
      ...
      expect(host.shadowRoot!.activeElement).to.equal(control);
      expect([control.selectionStart, control.selectionEnd, control.selectionDirection]).to.deep.equal([3, 8, "forward"]);
      expect(events).to.deep.equal([]);
    });
  }
  ...
  it("reconciles a nested color text edit into the parent value and FormData without events", async () => {
    ...
    `);
    const host = form.firstElementChild as HTMLElement & { updateComplete: Promise<boolean>; value: string };
  ...
  it("combines captured OTP boxes into canonical state without input or completion events", async () => {
    const form = await fixture<HTMLFormElement>(html`<form><fluid-otp name="code" length="4"></fluid-otp></form>`);
    const host = form.firstElementChild as HTMLElement & { updateComplete: Promise<boolean>; value: string };
    const controls = [...host.shadowRoot!.querySelectorAll("input")];
    ["1", "2", "3", "4"].forEach((value, index) => { controls[index]!.value = value; });
    controls[2]!.focus();
    const restore = captureFluidFormState(form);
  ...
  it("never assigns a file input's value or invokes its unsupported selection API", async () => {
    const input = await fixture<HTMLInputElement>(html`<input type="file" aria-label="Attachment">`);
    input.focus();
    const restore = captureFluidFormState(input);
    Object.defineProperty(input, "value", {
      configurable: true,
      get: () => "",
      set: () => { throw new Error("File value must not be assigned"); }
    });
    input.setSelectionRange = () => { throw new Error("File selection API must not be called"); };
    await restore();
    expect(document.activeElement).to.equal(input);
  });

  it("restores a number value without invoking its unsupported selection API", async () => {
    const input = await fixture<HTMLInputElement>(html`<input type="number" value="42" aria-label="Quantity">`);
    input.focus();
    const restore = captureFluidFormState(input);
    input.value = "7";
    input.setSelectionRange = () => { throw new Error("Number selection API must not be called"); };
    await restore();
  ...
  it("restores native multi-select, textarea, focus and text selection", async () => {
    const root = await fixture<HTMLDivElement>(html`<div>
      <select multiple aria-label="Choices"><option value="a" selected>A</option><option value="b" selected>B</option></select>
    ...
    expect([textarea.selectionStart, textarea.selectionEnd, textarea.selectionDirection]).to.deep.equal([2, 6, "backward"]);
  });

  it("does not mutate a replacement node when a captured control was removed", async () => {
    const root = await fixture<HTMLDivElement>(html`<div><input value="before" aria-label="Old control"></div>`);
    const input = root.querySelector("input")!;
    const restore = captureFluidFormState(root);
  ...
  it("does not replay captured state over later edits when restoration is called twice", async () => {
    const input = await fixture<HTMLInputElement>(html`<input value="captured" aria-label="Note">`);
    const restore = captureFluidFormState(input);
    await restore();
  ...
  it("shares one in-flight restoration promise across concurrent callers", async () => {
    const host = await fixture<FluidInput>(html`<fluid-input label="Note" value="server"></fluid-input>`);
    const input = host.shadowRoot!.querySelector("input")!;
    input.value = "captured";
    const restore = captureFluidFormState(host);
  ...
  it("rejects after exactly ten nonsettling host update results without applying captured state", async () => {
    ...
    Object.defineProperty(host, "updateComplete", {
      configurable: true,
      get: () => { reads++; return Promise.resolve(false); }
    });
