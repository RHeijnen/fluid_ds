...
export const renderCycleMessage = (tag: string) => `Element ${tag} scheduled an update (generally because a property was set) after an update completed, causing a new update to be scheduled. This is inefficient and should be avoided unless the next update can only be scheduled as a side effect of the previous update. See https://lit.dev/msg/change-in-update for more information.`;
const developmentNotice = "Lit is in dev mode. Not recommended for production! See https://lit.dev/msg/dev-mode for more information.";
...
export function captureErrors(page: Page): string[] {
  ...
  page.on("console", (message) => {
    ...
    if (message.type() === "warning") {
      const classification = text === developmentNotice ? "development-notice"
        : "unexpected";
