...
const localeRegistrations = import.meta.glob("../../../packages/components/src/locales/{nl,de,fr,es,ar}.ts");
...
window.addEventListener("unhandledrejection", (event) => recordHydrationMessage(String(event.reason?.message ?? event.reason)));
...
for (const target of [form, adoptionForm, compositeAdoptionForm]) {
  for (const type of ["fluid-input", "fluid-change"]) {
    target.addEventListener(type, (event) => {
      window.fluidFormEvents.push({ type, id: (event.target as HTMLElement).id, detail: (event as CustomEvent).detail });
    });
  }
}
...
const serverNodeIds = [
  "stateful", "choice", "amount", "contact", "action",
  "adopt-masked", "adopt-number", "adopt-slider", "adopt-switch", "adopt-textarea", "adopt-typeahead",
  "adopt-color", "adopt-date", "adopt-date-range", "adopt-otp", "adopt-tags", "adopt-time",
  "localized-pagination"
];
const serverNodes = serverNodeIds.map((id) => stateFixture.querySelector<HTMLElement>(`#${id}`)!).map((host) => ({
  host, root: host.shadowRoot, control: host.shadowRoot?.querySelector("input, textarea, button")
}));
window.assertFluidServerNodes = () => {
  for (const { host, root, control } of serverNodes) {
    if (!root || !control || !host.isConnected || host.shadowRoot !== root || !root.contains(control)) {
...
type UpdatingElement = HTMLElement & { updateComplete?: Promise<boolean>; isUpdatePending?: boolean };
...
async function finishUpdates(): Promise<void> {
  for (let pass = 0; pass < 10; pass++) {
    const results = await Promise.all(updatingElements(document).map((element) => element.updateComplete));
    if (results.every((result) => result !== false) && updatingElements(document).every((element) => !element.isUpdatePending)) return;
  }
...
async function hydrate(): Promise<void> {
  ...
  await finishUpdates();
  window.assertFluidServerNodes();
  if (window.fluidHydrationMismatches.length) throw new Error(window.fluidHydrationMismatches.join("\n"));
  window.fluidHydrated = true;
  window.dispatchEvent(new Event("fluid-hydrated"));
}

let hydration: Promise<void> | undefined;
window.hydrateFluid = () => hydration ??= hydrate().catch((error: unknown) => {
