...
for (const [tag, fixture] of fixtures) {
  test(`${tag} native legend keyboard, visibility, data update and reconnect`, async ({ page }) => {
    ...
    await page.goto(`/iframe.html?id=quality-chart-interaction-contracts--${fixture}-keyboard-fixture&viewMode=story`);
    ...
    await chart.evaluate((element) => {
      element.addEventListener("fluid-legend-change", (event) => {
        ...
        element.setAttribute("data-toggles", `${element.getAttribute("data-toggles") ?? ""}${detail.visible ? "1" : "0"}`);
      });
    });
    const visible = () => chart.evaluate((element) => {
      const host = element as FluidChart;
      return ["pie", "doughnut", "polarArea"].includes(host.type)
        ? host.instance!.getDataVisibility(0) : host.instance!.isDatasetVisible(0);
    });
    ...
    await expect(chart.getByRole("button", { name: arc ? "Updated North" : "Updated revenue", exact: true })).toBeVisible();
