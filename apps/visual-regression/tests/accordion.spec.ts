...
test.describe("Components/Accordion", () => {
  test("Default", async ({ page }) => {
    ...
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-accordion--default': ${sbErr.trim()}`);
    }
  ...
  test("Single", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-accordion--single&viewMode=story`);
    // Wait for Storybook to render the story root and for fonts + custom
    // elements to finish upgrading. We screenshot the full viewport rather
    // than the root element because some stories render off-screen overlays
    // (dialog, drawer, toast) whose root has zero visible bounds.
    ...
    const sbErr = await page.locator("#storybook-root-error-display, #error-message").first().textContent().catch(() => null);
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-accordion--single': ${sbErr.trim()}`);
    }
    await page.waitForFunction(() => {
      const root = document.getElementById("storybook-root");
      return !!root && root.childElementCount > 0;
    ...
    await page.waitForTimeout(150);
    await expect(page).toHaveScreenshot("single.png", {
      fullPage: false
    });
  });

  test("Disabled", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-accordion--disabled&viewMode=story`);
    // Wait for Storybook to render the story root and for fonts + custom
    // elements to finish upgrading. We screenshot the full viewport rather
    // than the root element because some stories render off-screen overlays
    // (dialog, drawer, toast) whose root has zero visible bounds.
    ...
    const sbErr = await page.locator("#storybook-root-error-display, #error-message").first().textContent().catch(() => null);
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-accordion--disabled': ${sbErr.trim()}`);
    }
    await page.waitForFunction(() => {
      const root = document.getElementById("storybook-root");
      return !!root && root.childElementCount > 0;
