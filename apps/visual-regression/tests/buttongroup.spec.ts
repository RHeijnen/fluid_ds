...
test.describe("Components/ButtonGroup", () => {
  test("Default", async ({ page }) => {
    ...
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-buttongroup--default': ${sbErr.trim()}`);
    }
  ...
  test("Vertical", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-buttongroup--vertical&viewMode=story`);
    // Wait for Storybook to render the story root and for fonts + custom
    // elements to finish upgrading. We screenshot the full viewport rather
    // than the root element because some stories render off-screen overlays
    // (dialog, drawer, toast) whose root has zero visible bounds.
    ...
    const sbErr = await page.locator("#storybook-root-error-display, #error-message").first().textContent().catch(() => null);
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-buttongroup--vertical': ${sbErr.trim()}`);
    }
    await page.waitForFunction(() => {
      const root = document.getElementById("storybook-root");
      return !!root && root.childElementCount > 0;
  ...
  test("PrimaryEnd", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-buttongroup--primary-end&viewMode=story`);
    // Wait for Storybook to render the story root and for fonts + custom
    // elements to finish upgrading. We screenshot the full viewport rather
    // than the root element because some stories render off-screen overlays
    // (dialog, drawer, toast) whose root has zero visible bounds.
    ...
    const sbErr = await page.locator("#storybook-root-error-display, #error-message").first().textContent().catch(() => null);
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-buttongroup--primary-end': ${sbErr.trim()}`);
    }
    await page.waitForFunction(() => {
      const root = document.getElementById("storybook-root");
      return !!root && root.childElementCount > 0;
    ...
    await page.waitForTimeout(150);
    await expect(page).toHaveScreenshot("primary-end.png", {
      fullPage: false
    });
  });
});
