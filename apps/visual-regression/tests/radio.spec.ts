...
test.describe("Components/Radio", () => {
  test("Vertical", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-radio--vertical&viewMode=story`);
    ...
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-radio--vertical': ${sbErr.trim()}`);
    }
  ...
  test("Horizontal", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-radio--horizontal&viewMode=story`);
    // Wait for Storybook to render the story root and for fonts + custom
    // elements to finish upgrading. We screenshot the full viewport rather
    // than the root element because some stories render off-screen overlays
    // (dialog, drawer, toast) whose root has zero visible bounds.
    ...
    const sbErr = await page.locator("#storybook-root-error-display, #error-message").first().textContent().catch(() => null);
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-radio--horizontal': ${sbErr.trim()}`);
    }
    await page.waitForFunction(() => {
      const root = document.getElementById("storybook-root");
      return !!root && root.childElementCount > 0;
  ...
  test("WithDisabled", async ({ page }) => {
    await page.goto(`/iframe.html?id=components-radio--with-disabled&viewMode=story`);
    // Wait for Storybook to render the story root and for fonts + custom
    // elements to finish upgrading. We screenshot the full viewport rather
    // than the root element because some stories render off-screen overlays
    // (dialog, drawer, toast) whose root has zero visible bounds.
    ...
    const sbErr = await page.locator("#storybook-root-error-display, #error-message").first().textContent().catch(() => null);
    if (sbErr) {
      throw new Error(`Storybook failed to render story 'components-radio--with-disabled': ${sbErr.trim()}`);
    }
    await page.waitForFunction(() => {
      const root = document.getElementById("storybook-root");
      return !!root && root.childElementCount > 0;
