...
const fixtureDirectory = join(root, "apps/framework-vue");
...
async function assertPackedVueConsumer(directory) {
...
export async function runRuntime(consumerDirectory, evidenceDirectory) {
  const consumer = await assertPackedVueConsumer(consumerDirectory);
  ...
  try {
    for (const engine of engines) {
      ...
      try {
            ...
            `${JSON.stringify(
              {
                ...
                assertion:
                  "The production HTTP response contains Fluid SSR or declarative shadow DOM",
                fluidHostCount: (markup.match(/<fluid-/g) ?? []).length,
                declarativeShadowRootCount: (markup.match(/shadowrootmode=/g) ?? []).length,
                cause:
                  "The designated Vue fixture uses createApp with a Vite client entry and has no server renderer."
              },
              null,
              2
            )}\n`
        ...
        assert.match(markup, /<div id="app"><\/div>/);
        ...
        await page.waitForFunction(() => window.vueFluid?.ready === true);
        assert.deepEqual(await page.evaluate(() => window.vueFluid.definitionsBeforeRegistration), {
          "fluid-button": false,
          "fluid-card": false,
        ...
        assert.deepEqual(await page.evaluate(() => window.vueFluid.references()), {
          input: "fluid-input",
        ...
        await page.evaluate(() => window.vueFluid.register());
        await page.waitForFunction(
          () => window.vueFluid.registered || window.vueFluid.registrationError !== null
        );
        assert.equal(await page.evaluate(() => window.vueFluid.registrationError), null);
        ...
        assert.deepEqual(
          ...
          { value: "Vue serverless consumer", prefix: ["slot-prefix"] }
        );
        await expect(page.locator("#approved input")).toBeChecked();

        stage = "properties-events-refs-forms";
        await page.evaluate(() => window.vueFluid.setProject("Set through Vue state"));
        await expect(page.locator("#project input")).toHaveValue("Set through Vue state");
        await page.evaluate(() => window.vueFluid.setLabel("Updated through element ref"));
        await expect(page.locator("#project label")).toContainText("Updated through element ref");
        const input = page.locator("#project input");
        await input.fill("Typed in Vue");
        await input.blur();
        assert.deepEqual(await page.evaluate(() => window.vueFluid.events.slice(0, 2)), [
          { type: "fluid-input", detail: { value: "Typed in Vue" } },
          { type: "fluid-change", detail: { value: "Typed in Vue" } }
        ]);
        await page.locator("#approved label").click();
        await expect(page.locator("#approved input")).not.toBeChecked();
        assert.deepEqual(await page.evaluate(() => window.vueFluid.events.at(-1)), {
          type: "fluid-change",
        ...
        await expect(page.locator("#contract-output")).toHaveText(
          '[["project","Typed in Vue"],["approved","yes"]]'
        );
        assert.deepEqual(await page.evaluate(() => window.vueFluid.submissions), [
          [
            ["project", "Typed in Vue"],
            ["approved", "yes"]
          ]
        ]);
        ...
        assert.deepEqual(errors, [], "Vue page emitted console, page, network or HTTP errors");
...
async function main() {
  ...
  const evidenceDirectory = join(root, "quality/evidence/framework-vue", timestamp);
  const tempRoot = await mkdtemp(join(tmpdir(), "fluid-vue-packed-"));
  ...
  try {
    ...
    manifest.name = "@fluid-ds/framework-vue-packed-contract";
    ...
    await assertPackedVueConsumer(consumerDirectory);
    ...
    assert.equal(runtime.status, "passed", "Packed Vue runtime contract failed");
    status = "passed";
  } finally {
    await mkdir(evidenceDirectory, { recursive: true });
      ...
      `${JSON.stringify(
        {
          ...
          limitations: [
            "This Vite fixture is client-side rendered; it provides no SSR HTML or framework hydration contract.",
    ...
    console.log(`Retained packed Vue evidence: ${evidenceDirectory}`);
    if (tempRoot.startsWith(`${tmpdir()}${sep}fluid-vue-packed-`)) {
