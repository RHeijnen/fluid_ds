...
export function Users() {
  ...
  const dialogRef = useRef<FluidDialog>(null);
  const nameRef = useRef<FluidInput>(null);
  const emailRef = useRef<FluidInput>(null);
  const roleRef = useRef<FluidSelect>(null);

  // Custom event from the typeahead → React state (the bridge between the web
  // component's events and React's data flow).
  const searchRef = useFluidEvent<FluidTypeahead>("fluid-input", (e) =>
