...
import type { FluidDoughnutChart, FluidLineChart } from "@fluid-ds/charts";

/** The Fluid ripples mark. */
...
/** Bind a custom event (e.g. `fluid-change`) on a Fluid element via a ref. */
...
/** Push a toast onto the app's <fluid-toast> stack. */
...
type ChartProperties = HTMLElement & { data: unknown; options?: unknown };

/** React wrapper: Chart.js `data`/`options` are objects, so set them as element
 * properties through a ref. (Properties set before the element upgrades are
 * preserved by Lit, which matters under SSR + deferred client registration.) */
export function FluidChart({
  ...
  return kind === "doughnut" ? (
    <fluid-doughnut-chart ref={doughnutRef} style={style} />
  ) : (
    <fluid-line-chart ref={lineRef} style={style} />
  );
}
