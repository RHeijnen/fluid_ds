...
const fixtureDirectory = join(root, "apps/admin-next");
...
async function assertPackedNextConsumer(directory) {
  ...
  for (const name of packageNames) {
    ...
    if (["@fluid-ds/components", "@fluid-ds/react", "@fluid-ds/charts"].includes(name)) {
      assert.doesNotMatch(
...
async function startNextExportServer(directory) {
...
export async function runRuntime(consumerDirectory, evidenceDirectory, providedServer) {
  const consumer = await assertPackedNextConsumer(consumerDirectory);
  ...
  const server = providedServer ?? (await startNextExportServer(join(consumer, "out")));
  const results = [];
  const failures = [];
  const cleanup = [];
  ...
  try {
    for (const engine of engines) {
      ...
      try {
        ...
        const response = await page.goto(`${server.origin}/ssr-contract/`);
        ...
        assert.equal(
          ...
          4,
          "Next export must contain four Fluid declarative shadow roots"
        );
        assert.match(
          markup,
        ...
        await page.waitForFunction(() => window.nextFluid?.ready === true);
        assert.deepEqual(
          await page.evaluate(() => window.nextFluid.definitionsBeforeRegistration),
          {
            "fluid-button": false,
            "fluid-card": false,
        ...
        assert.equal(await page.evaluate(() => window.nextFluid.assertServerNodes()), 4);
        assert.deepEqual(await page.evaluate(() => window.nextFluid.slotAssignments()), {
          header: ["slot-header"],
          body: ["contract-form"],
          footer: ["slot-footer"],
          prefix: ["slot-prefix"]
        ...
        assert.deepEqual(await page.evaluate(() => window.nextFluid.events), []);

        stage = "registration-and-hydration";
        await page.evaluate(() => window.nextFluid.register());
        await page.waitForFunction(
          () => window.nextFluid.hydrated || window.nextFluid.hydrationError !== null
        );
        assert.equal(await page.evaluate(() => window.nextFluid.hydrationError), null);
        assert.equal(await page.evaluate(() => window.nextFluid.assertServerNodes()), 4);
        assert.deepEqual(await page.evaluate(() => window.nextFluid.slotAssignments()), {
          header: ["slot-header"],
          body: ["contract-form"],
        ...
        assert.deepEqual(await page.evaluate(() => window.nextFluid.submissions), [
          [
        ...
        assert.deepEqual(
          await page.evaluate(() => window.nextFluid.events.at(-1)),
          { type: "fluid-input", value: "Edited after hydration" },
          "Hydrated Fluid input event did not cross the Next client boundary"
        );
        ...
        assert.equal(await page.evaluate(() => window.nextFluid.assertServerNodes()), 4);
        assert.deepEqual(errors, [], "Next page emitted console, page, network or HTTP errors");
    ...
    if (!providedServer) await server.close();
  }
  ...
  const report = {
    ...
    mode:
      providedServer?.mode ??
      "packed-production-static-export-with-server-DSD-and-delayed-client-registration",
    engines,
    results,
    failures,
    cleanup
...
async function main() {
  ...
  const evidenceDirectory = join(root, "quality/evidence/framework-next", timestamp);
  const tempRoot = await mkdtemp(join(tmpdir(), "fluid-next-packed-"));
  ...
  try {
    ...
    manifest.name = "@fluid-ds/admin-next-packed-contract";
    manifest.packageManager = "pnpm@9.15.0";
    manifest.scripts = Object.fromEntries(
      Object.entries(manifest.scripts ?? {}).filter(([name]) => !name.startsWith("pre"))
    );
    manifest.pnpm = {
      ...(manifest.pnpm ?? {}),
    ...
    await assertPackedNextConsumer(consumerDirectory);
    ...
    assert.equal(runtime.status, "passed", "Packed Next runtime contract failed");
    status = "passed";
  } finally {
    await mkdir(evidenceDirectory, { recursive: true });
      ...
      `${JSON.stringify(
        {
          ...
          limitations: [
            "The designated Next fixture uses output: export; DSD is server-rendered during next build and served as production static HTML, not generated by an adapter on each request.",
            ...
            "Package tarballs use the repository's existing built dist; source-to-dist equivalence remains covered by separate package build and artifact gates."
          ]
        },
        null,
        2
    ...
    console.log(`Retained packed Next evidence: ${evidenceDirectory}`);
    if (tempRoot.startsWith(`${tmpdir()}${sep}fluid-next-packed-`)) {
