...
const fixtureDirectory = join(root, "apps/framework-astro");
...
async function assertPackedAstroConsumer(directory) {
  ...
  for (const name of ["@fluid-ds/components", "@fluid-ds/tokens"]) {
    ...
    if (name === "@fluid-ds/components") {
      ...
      assert.match(exports, /\.\/dist\/ssr\.js/, "Packed components SSR entry is not built JS");
      assert.match(
        exports,
        /\.\/dist\/ssr-client\.js/,
        "Packed components SSR client entry is not built JS"
      );
    }
  }
...
export async function runRuntime(consumerDirectory, evidenceDirectory) {
  const consumer = await assertPackedAstroConsumer(consumerDirectory);
    ...
    for (const engine of engines) {
      ...
      try {
        ...
        assert.equal(
          ...
          4,
          "Astro response must contain four Fluid declarative shadow roots"
        );
        assert.match(
          markup,
        ...
        await page.waitForFunction(() => window.astroFluid?.ready === true);
        assert.deepEqual(
          await page.evaluate(() => window.astroFluid.definitionsBeforeRegistration),
          {
            "fluid-button": false,
            "fluid-card": false,
        ...
        assert.equal(await page.evaluate(() => window.astroFluid.assertServerNodes()), 4);
        assert.deepEqual(await page.evaluate(() => window.astroFluid.slotAssignments()), {
          header: ["slot-header"],
          body: ["contract-form"],
          footer: ["slot-footer"],
          prefix: ["slot-prefix"]
        ...
        await page.evaluate(() => window.astroFluid.register());
        await page.waitForFunction(
          () => window.astroFluid.hydrated || window.astroFluid.hydrationError !== null
        );
        assert.equal(await page.evaluate(() => window.astroFluid.hydrationError), null);
        assert.equal(await page.evaluate(() => window.astroFluid.assertServerNodes()), 4);
        assert.deepEqual(await page.evaluate(() => window.astroFluid.slotAssignments()), {
          header: ["slot-header"],
          body: ["contract-form"],
        ...
        assert.deepEqual(await page.evaluate(() => window.astroFluid.events), []);
        ...
        assert.deepEqual(await page.evaluate(() => window.astroFluid.submissions), [
          [
        ...
        assert.equal(
          await page.evaluate(() =>
            window.astroFluid.events.some(
              (event) => event.type === "fluid-input" && event.value === "Edited after hydration"
            )
          ),
          true,
          "Hydrated Fluid input event did not cross the Astro boundary"
        );
        ...
        assert.equal(await page.evaluate(() => window.astroFluid.assertServerNodes()), 4);
        assert.deepEqual(errors, [], "Astro page emitted console, page, network or HTTP errors");
  ...
  const report = {
    status:
      ...
      results.every((result) => result.status === "passed")
        ...
        : "failed",
    framework: "Astro",
    mode: "packed-production-static-SSR-with-DSD-and-delayed-client-registration",
    engines,
    results,
    failures,
    cleanup
...
async function main() {
  ...
  const evidenceDirectory = join(root, "quality/evidence/framework-astro", timestamp);
  const tempRoot = await mkdtemp(join(tmpdir(), "fluid-astro-packed-"));
  ...
  try {
    ...
    manifest.name = "@fluid-ds/framework-astro-packed-contract";
    ...
    await assertPackedAstroConsumer(consumerDirectory);
    ...
    assert.equal(runtime.status, "passed", "Packed Astro runtime contract failed");
    status = "passed";
  } finally {
    await mkdir(evidenceDirectory, { recursive: true });
    ...
    console.log(`Retained packed Astro evidence: ${evidenceDirectory}`);
    if (tempRoot.startsWith(`${tmpdir()}${sep}fluid-astro-packed-`)) {
